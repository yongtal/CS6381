...
#
# Purpose:
# This code runs the wordcount reduce task. It runs inside the worker process. Since the
...
class MR_Reduce ():
    ...
    def do_work (self):
        ...
        # Wait for that nudge -- very important to understand the mechanism
        ...
        # Note: use the python pickle package to save/load of in-memory data structure like
        # list of list into file. This makes it very simple to do the reduce activity.
        ...
        for items in contents:
            #print items
            reduce_file.write(items[0][0] + ",")
            result = 0
            # now let us sum up the results per word
            for j in range (len (items)):
                result += int (items[j][1])
                totalprs += int (items[j][1]) 
        ...
        print 
        print "Total (Key, Value) Pair Processed:",totalprs
        print 
