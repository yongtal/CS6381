...
# Note: this is the code runs inside the barrier sink thread!
...
class MR_Framework ():
    ...
    def distribute_reduce_tasks (self):
        ...
        # Note: like "map", the real work is done inside reducer's "mr_reduceworker" file.
        try:
    ...
    def shuffle_func (self):
        ...
        for i in range (self.M):
            ...
            worklist = sorted(csvfile, key=operator.itemgetter(0))
            ...
            for k, g in itertools.groupby (worklist, key=operator.itemgetter(0)):
                groups.append(list(g))
                uniquekeys.append(k)

            # Now create a temp file with combiner optimization
            shufflefile = open("Shuffletmp" + str(i) + ".csv", "w")
              
            localhist = {}
            for j in range(len(uniquekeys)):
                shufflefile.write (uniquekeys[j] + ", ")
                shufflefile.write (str (len (groups[j])) + "\n")

                localhist[uniquekeys[j]] = len(groups[j])
                sumCards += len(groups[j])

            shufflefile.close()

            mappersinfo.append(localhist)
        ...
        for i in range(self.M):
            shuffle_file = open("Shuffletmp" + str(i) + ".csv", "r")
        ...
        #print "sumCards", sumCards
        #print [i for i in mappersinfo]

        sumAvg = 0
        numClusters = len(self.uniquekeys)
        glpy = float(sumCards) / numClusters
        localpy = glpy / self.M
        for k in self.uniquekeys:
          upperG[k] = 0 
          lowerG[k] = 0 
        for i in range (self.M):
          for k in self.uniquekeys:
            if k in mappersinfo[i]:
              if mappersinfo[i][k] > localpy:
                upperG[k] += mappersinfo[i][k]
                lowerG[k] += mappersinfo[i][k]
              else :
                upperG[k] += localpy
        for k in upperG: 
          avg = (upperG[k]+lowerG[k])/2.0
          if avg > glpy:
            rG[k] = avg
            sumAvg += avg
        ...
        estPy = (sumCards - sumAvg) / (numClusters - len(rG))
        #print "group", [ i for i in self.groups]
        #print "uG", [ (i,upperG[i]) for i in upperG]
        #print "lG", [ (i,lowerG[i]) for i in lowerG]
        #print "rG", [ (i,rG[i]) for i in rG]
        #print "sumAvg", sumAvg
        #print "estPy", estPy 


        rsizes = [1.0*sumCards / self.R] * self.R
        shuffle_conts = []
        for i in range(self.R):
          shuffle_conts.append([])
        
        pos = 0
        for klst in self.groups:
          k = klst[0][0]
          if k in rG:
            costk = rG[k]
          else:
            costk = estPy 
          #print "k is",k,"cost",str(costk)
          for i in range (self.R):
            if rsizes[i] - costk > 0 or i == self.R-1:
              rsizes[i] -= costk
              shuffle_conts[i].append(self.groups[pos])
              break
          pos += 1 
        for i in range(self.R):
          shuffle_file = open ("Shuffle" + str(i) + ".dat", "wb")
          pickle.dump(shuffle_conts[i], shuffle_file, pickle.HIGHEST_PROTOCOL)
          shuffle_file.close()

        # now save the shuffle info into as many files as the number
        # of reduce jobs
        # Note: in Assignment3, this step is done at the beginning of reduce method not here.
        ...
        #    pickle.dump(self.groups[keysConsumed:keysConsumed + keysPerReduce], shuffle_file, pickle.HIGHEST_PROTOCOL)
    ...
    def finalize_func(self):
        ...
        # effectively, we go through all the reduce results files and
        # get the results
        temp_file = open ("temp.csv", "w")
        ...
        csvfile = csv.reader(open("temp.csv", "r"), delimiter=",")
        csvdata = ((i[0],int(i[1])) for i in csvfile)
        worklist = sorted(csvdata, key=operator.itemgetter(1),reverse=True)
        #del csvdata
        for k, v in worklist:
	...
	for i in range(self.M):
	    os.remove("Shuffletmp" + str(i) + ".csv")
        for i in range(self.R):
    ...
    def solve (self):
        ...
        try:
            ...
            ### barrier ###
            # Note: here is how to use the barrier to synchronous the map operation!
            # You wait the "map" barrier thread to return
            ...
            ### barrier ###
            # Note: here is how to use the barrier to synchronous the return operation!
            # You wait the "return" barrier thread to return
