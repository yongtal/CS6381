...
def parseCmdLineArgs ():
    ...
    parser.add_argument ("-M", "--map", type=int, default=2, help="Number of Map jobs, default 10")
...
def genCommandsFile (hosts, args):
    try:
        ...
        cmd_str = hosts[0].name + " python server.py " + " &> " + hosts[0].name + ".out &\n"
        cmds.write (cmd_str)

        #  next create the command for the map workers
        for i in range (args.map):
            cmd_str = hosts[i+1].name + " python publisher.py " + " " + hosts[i+1].IP() + " &> " + hosts[i+1].name + ".out &\n"
            cmds.write (cmd_str)
